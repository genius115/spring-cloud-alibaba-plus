...
public class Consumer {
    ...
    private static String ROUTING_KEY_WARNING="ROUTING-WARNING";
    ...
    public static void main(String[] args) throws IOException {
        ...
        channel.queueBind(queue,EXCHANGE_NAME,ROUTING_KEY_ERROR);
        channel.queueBind(queue,EXCHANGE_NAME,ROUTING_KEY_WARNING);

        //消费消息
        channel.basicConsume(queue,true,new DefaultConsumer(channel){
            @Override
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                log.info("消费者1（info/error/Warning）："+new String(body));
            }
        });
